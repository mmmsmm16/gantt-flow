...
- **付随オブジェクトの配置は種類で非対称**（`03-view-spec.md` §2-3）:
  - **帳票/情報（I/O）**: 親工程ノードの**角に重ねて**配置 — 入力は**左上**、出力は**右下**。
    複数ある場合はその角から外側へ少しずつずらして積む。工程との重なりは許容（コンパクトに紐づけるため）。
  - **課題・コメント**: 親/対象の近傍から始め、**他オブジェクト（タスク・I/O・エッジ・他の課題）と重なる場合は
    空きが見つかるまで外側へオフセット**（ベストエフォートの衝突回避）。重なりは最小化する。
- 決定論であること（テストのため）。同じ入力なら同じ座標。手動で動かした座標は以後 `autoPlace` を上書きしない（保持）。
