...
- **主戦場はフローキャンバス（ユーザ確定・2026-07-05）**: フロー系提案（工程追加・依存・
  マイルストーン・担当/レーン変更）は、**フロー上に「仮ノード/仮エッジ」として直接描画**し、
  その場で承認/否認/修正する。カードリストは補助（全提案の一覧・非フロー系の判定場所）。
  - プレビューは `runBatch(現project, 未却下の提案)` → reconcile した preview project を
    専用のプレビュー描画（scenarioFlow/FlowCompareView の語彙を流用）で表示。
    仮=破線＋半透明（琥珀系）、承認済み=実線（緑トーン）、既存工程=減光、却下=非表示。
  - 仮ノードをクリック → ポップオーバーで [承認] [否認] [修正]。修正=ノード上でインライン
    リネーム（v1 は名称・担当。それ以外の修正はカード側で）。
  - 提案→ノードの対応は BatchResult の aliases/created id で機械的に取れる（マッピング層を持つ）。
  - 判定はカードとフローで**同一の decisions 状態を共有**（どちらで操作しても即同期）。
  - 却下の波及（ref-DAG）はフロー上でも即反映（下流の仮ノードが「無効」表示に）。
- AI パネル（ドロワー）: メモ貼り付け → 生成（ストリーミング進捗）→ カードリスト（補助）＋
  「フローで確認」トグル → 適用バー「承認 N 件を適用（undo で戻せます）」。
- 起動導線: ツールバー（AI 有効時のみ表示）＋コマンドパレット＋手順書タブの「✨ドラフト生成」
  ボタン（既存の disabled を活性化 — 対象工程を固定した procedureDraft モード）。
- 手順書ドラフト・set_detail・入出力・課題などフローに置けない提案はカードで判定
  （対象工程ノードに小バッジを出して存在は示す）。
...
- desktop: プロバイダ層はネットワークをモックして「組み立てた リクエストの形」を検証
  ...
  プレビュー再計算の決定論（同じ decisions → 同じ preview project）・提案⇔ノード対応の単体テスト。
...
3. desktop: 提案 UI（フロー上の仮ノード承認=主戦場／カード=補助／ref-DAG／適用/undo）＋手順書タブ連携
