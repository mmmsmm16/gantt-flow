...
## v2: 粒度非依存化（2026-07-04 ユーザ指摘による設計変更）

「マイルストーンは工程粒度に左右されない」— 節目は業務の事実であり、大・中・小・詳細の
**すべてのビューに同じマイルストーンが出る**ように変更する。

- **表示規則**: 全スコープビューには常に表示。スコープ付きビューは、対象工程のいずれかが
  そのスコープ配下にあるときのみ表示。未紐付け MS は全スコープビューのみ。
- **縦線の位置 = 代表ノードへの自動変換**: 対象工程がそのビューに居ないときは
  ①対象工程より細かいビュー → 対象工程の**子孫でビューに見えているノード群の右端の最大値**
  ②粗いビュー → 対象工程の**そのレベルの祖先ノード**の右端。x = max(代表右端) + 40。
- **ノード**: reconcile の対象選定（targets）でマイルストーンを level 不問で含める
  （1:1 不変条件はビューごとに維持。未紐付けの手動 x はビューごとに独立 — 既存の
  「配置はレベルごと」原則と同じ）。
- `ProcessTask.level` は MS では無意味化（フィールドは残すが全箇所で無視）。表の粒度セルは
  「—」・編集不可。対象工程（前工程）候補は MS 行では**レベル不問**で選べるように広げる。
- 依存の「同一レベル・同一スコープ」前提は MS 依存には適用しない（導出エッジ・tidy からは
  既に隔離済みのため安全）。
