...
describe('付箋の対象工程リンク（課題線と同じ細い薄線・ダングリング禁止）', () => {
  it('対象工程があれば付箋中心 → 工程中心へ issueLine 色の線を描く', () => {
    const p = createSampleProject(counter());
    const view = mediumView(p);
    const task = firstTaskNode(p);
    view.nodes['cmt1'] = {
      id: 'cmt1',
      kind: 'comment',
      text: 'メモ',
    ...
    const svg = buildFlowSvg(p, view);
    const c = issueLineTarget(task, Object.values(view.nodes), p.details);
    // 付箋(100x44)の中心 (550,522) から工程中心へ、課題線と同じ色・太さの矢頭なし線。
    expect(svg).toContain(
      `<line x1="550" y1="522" x2="${c.x}" y2="${c.y}" stroke="${FLOW_LIGHT.issueLine}" stroke-width="1"/>`,
    );
  });

  it('対象ノードが存在しなければ線を描かない（ダングリング描画禁止）', () => {
    const p = createSampleProject(counter());
    const view = mediumView(p);
    view.nodes['cmt2'] = {
      id: 'cmt2',
      ...
      x: 500,
      y: 500,
      targetNodeId: 'no-such-node',
    };
    const svg = buildFlowSvg(p, view);
    expect(svg).not.toContain('x1="550" y1="522"'); // 付箋中心を始点にした線は無い
  });
});

describe('buildFlowSvg のハイライト(opts.highlightTaskIds)', () => {
  it('opts 省略時と opts:{}（highlightTaskIds 無指定）は同一出力＝従来どおり', () => {
    const p = createSampleProject(counter());
    const view = mediumView(p);
    expect(buildFlowSvg(p, view)).toBe(buildFlowSvg(p, view, {}));
  });

  it('対象タスクノードはアクセント太枠＋ハローが付き、非対象タスク/エッジ/レーンは減光(opacity 0.4)される', () => {
    const p = createSampleProject(counter());
    const view = mediumView(p);
    const taskNodes = Object.values(view.nodes).filter(
      (n): n is Extract<FlowNode, { kind: 'task' }> => n.kind === 'task',
    );
    expect(taskNodes.length).toBeGreaterThanOrEqual(2); // 受注業務スコープ = 注文受付/与信確認/在庫引当
    const target = taskNodes[0]!;
    const other = taskNodes[1]!;

    const plainSvg = buildFlowSvg(p, view);
    const svg = buildFlowSvg(p, view, { highlightTaskIds: new Set([target.taskId]) });

    // ハイライト無指定時には存在しないアクセント太枠・ハローが、指定時にのみ現れる。
    expect(plainSvg).not.toContain('stroke-width="2.5"');
    expect(svg).toContain('stroke="#0e6f6a" stroke-width="2.5"'); // アクセントの太枠
    expect(svg).toContain('rgba(14,111,106'); // わずかなハロー

    // 非対象タスクの矩形は opacity="0.4" で減光される。
    expect(svg).toMatch(
      new RegExp(`<rect x="${other.x}" y="${other.y}"[^>]*stroke-width="1.5" opacity="0.4"/>`),
    );
    // 対象タスクの矩形自体は減光されない(opacity 属性が付かない)。
    expect(svg).not.toMatch(
      new RegExp(`<rect x="${target.x}" y="${target.y}"[^>]*stroke-width="2.5" opacity=`),
    );

    // エッジは対象/非対象を問わず一律減光される。
    expect(plainSvg).not.toContain('marker-end="url(#a)" opacity="0.4"');
    expect(svg).toContain('marker-end="url(#a)" opacity="0.4"');
  });

  it('ハイライト集合が空でも例外にならず、全タスクが減光される', () => {
    const p = createSampleProject(counter());
    const view = mediumView(p);
    const svg = buildFlowSvg(p, view, { highlightTaskIds: new Set() });
    expect(svg).not.toContain('stroke-width="2.5"'); // アクセント太枠は現れない
    expect((svg.match(/opacity="0.4"/g) ?? []).length).toBeGreaterThan(0);
  });
});
