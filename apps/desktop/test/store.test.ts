...
describe('updateDetail: 状況（ヒアリング進行）の編集と undo', () => {
  it('status を設定でき、undo で元に戻る', () => {
    const s = createAppStore();
    s.getState().addTask('受付');
    const id = idByName(s, '受付');
    expect(s.getState().project.details[id]?.status).toBeUndefined();

    s.getState().updateDetail(id, { status: 'heard' });
    expect(s.getState().project.details[id]?.status).toBe('heard');

    s.getState().undo();
    expect(s.getState().project.details[id]?.status).toBeUndefined();
  });

  it('status: undefined でクリアできる', () => {
    const s = createAppStore();
    s.getState().addTask('受付');
    const id = idByName(s, '受付');
    s.getState().updateDetail(id, { status: 'review' });
    expect(s.getState().project.details[id]?.status).toBe('review');

    s.getState().updateDetail(id, { status: undefined });
    expect(s.getState().project.details[id]?.status).toBeUndefined();
  });
});
