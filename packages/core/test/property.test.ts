...
describe('プロパティテスト（ランダムなコマンド列で不変条件）', () => {
  ...
  it('逐次 reconcile＋新規配置は「配置時点で既存と重ならない」＋冪等（衝突回避ブランチの fuzz）', () => {
    const overlap = (a: FlowTaskNode, b: FlowTaskNode) =>
    ...
    fc.assert(
      fc.property(
        ...
        (ops, perturb) => {
          let p = ensureLevelView(emptyProject(), 'medium');
          // 1 op ごとに reconcile（＝実アプリの「編集のたび同期」）。合間に既存ノードを手で動かす
          // （ドラッグ相当・格子を跨ぐ位置へ）。手動移動は衝突を誘発する“既存レイアウト”を作る役。
          ops.forEach((op, i) => {
            const beforeView = p.flow.byLevel.find(
              (v) => v.level === 'medium' && v.scopeParentId === undefined,
            );
            const before = beforeView
              ? Object.values(beforeView.nodes)
                  .filter((n): n is FlowTaskNode => n.kind === 'task')
                  .map((n) => ({ ...n }))
              : [];
            p = applyOp(p, op);
            const r = reconcileProject(ensureLevelView(p, 'medium'), uuid);
            p = r;
            // このパスで新規追加されたノード（before に無い taskId）は、配置時点の既存
            // （before）と矩形的に重ならないこと＝衝突回避の本旨。
            const view = p.flow.byLevel.find(
              (v) => v.level === 'medium' && v.scopeParentId === undefined,
            );
            if (view) {
              const nds = Object.values(view.nodes).filter((n): n is FlowTaskNode => n.kind === 'task');
              const beforeIds = new Set(before.map((n) => n.taskId));
              for (const nd of nds) {
                if (beforeIds.has(nd.taskId)) continue; // 新規のみ検査
                for (const b of before) expect(overlap(nd, b)).toBe(false);
              }
              // ドラッグ相当の手動移動（次パスの“既存レイアウト”を荒らす）。
              if (nds.length && perturb.length) {
                const target = nds[perturb[i % perturb.length]! % nds.length]!;
                target.x = 220 + (perturb[i % perturb.length]! % 8) * 110; // 110=COL_W/2 で跨がせる
                target.y = 80;
              }
            }
          });
          // 冪等（もう一度 reconcile しても不変）＝既存ノード位置も動かない。
          p = reconcileProject(ensureLevelView(p, 'medium'), uuid);
          const again = reconcileProject(p, uuid);
          expect(again).toEqual(p);
          return true;
        },
