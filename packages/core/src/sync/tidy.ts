...
export function tidyFlowView(
  ...
  view: FlowLevelView,
  // keepFixed: ここに含まれる工程ノードは pinned 同様レイアウト対象から外し位置を保つ。
  // 「選択した工程だけ整列し、他は触らない」（部分整列）用。指定時はレーン高さも再計算しない。
  keepFixed?: ReadonlySet<string>,
): FlowLevelView {
  ...
  // 全体ビューでは「前の親の末尾 → 次の親の先頭」がブリッジ矢印で繋がって見えるのに、コア依存
  // だけで段組みすると次の親の先頭が「前工程なし」と扱われて左端へ寄ってしまうため。
  const taskIdByNode = new Map(taskNodes.map((n) => [n.id, n.taskId]));
  const pairKey = (f: Id, t: Id) => `${f}\x00${t}`;
  const seenPairs = new Set(deps.map((d) => pairKey(d.from, d.to)));
  const layerLinks: Array<{ from: Id; to: Id }> = [...deps];
  for (const e of Object.values(next.edges)) {
    if (e.role === 'ioLink') continue;
    const f = taskIdByNode.get(e.source);
    const t = taskIdByNode.get(e.target);
    if (!f || !t || f === t || seenPairs.has(pairKey(f, t))) continue;
    seenPairs.add(pairKey(f, t));
    layerLinks.push({ from: f, to: t });
  }
  ...
  for (const n of taskNodes) {
    if (layerLinks.some((d) => d.to === n.taskId)) continue;
    const succLayers = layerLinks
      .filter((d) => d.from === n.taskId)
      .map((d) => layer.get(d.to) ?? 0);
  ...
  //  ・無依存(孤立) … 前後関係（ブリッジ含む）が一切無い工程 → 縦に積まずに、置いた位置の
  //    まま残す（謎の縦整列を防ぐ）。
  ...
  // 部分整列（keepFixed 指定）では既存のレーン高さ・固定ノード位置を壊さないよう再計算しない。
  if (!keepFixed) {
    for (const lane of Object.values(next.lanes)) {
